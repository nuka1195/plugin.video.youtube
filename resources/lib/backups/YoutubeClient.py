...
class YoutubeClient:
    ...
    if ( DEBUG ):
        ...
        BASE_COOKIE_PATH = os.path.join( xbmc.translatePath( xbmcaddon.Addon( id=os.path.basename( os.getcwd() ) ).getAddonInfo( "Profile" ) ), "cookie.txt" )
    ...
    def get_details( self, video_id, **params ):
        try:
            ...
            title = details[ "entry" ][ "title" ][ "$t" ]##.decode( encoding )
            ...
            author = details[ "entry" ][ "author" ][ 0 ][ "name" ][ "$t" ]##.decode( encoding )
            # genre
            genre = details[ "entry" ][ "media$group" ][ "media$category" ][ 0 ][ "$t" ]##.decode( encoding )
            # viewer rating
            try:
            ...
            if ( "media$description" in details[ "entry" ][ "media$group" ] ):
                ...
                plot = details[ "entry" ][ "media$group" ][ "media$description" ][ "$t" ]##.decode( encoding )
            # return our values
